...
NSString* lastAction = @"";
NSString *urlString = @"https://api.zoomauth.com/api/v1/biometrics";
...
@implementation ZoomManagedSessionFaceMapProcessor {
    ...
    id<ZoomFaceMapResultCallback> latestFaceMapResultCallback;
...
- (void)processZoomSessionResultWhileZoomWaits:(id<ZoomSessionResult> _Nonnull)zoomSessionResult zoomFaceMapResultCallback:(id<ZoomFaceMapResultCallback> _Nonnull)zoomFaceMapResultCallback {
    ...
    if(zoomSessionResult.status != ZoomSessionStatusSessionCompletedSuccessfully) {
        if(zoomSessionResult.status == ZoomSessionStatusLandscapeModeNotAllowed || zoomSessionResult.status == ZoomSessionStatusReversePortraitNotAllowed) {
        ...
        else if(zoomSessionResult.status == ZoomSessionStatusCameraPermissionDenied) {
    ...
    latestFaceMapResultCallback = zoomFaceMapResultCallback;
    ...
    if(faceMetrics.faceMap == nil) {
        ...
        return;
            ...
            [zoomManagedSession setZoomManagedSessionStatusSubCode:ZoomManagedSessionStatusSubCodeCompletedSuccessfully];
...
- (void) URLSession:(NSURLSession *)session task:(NSURLSessionTask *)task didSendBodyData:(int64_t)bytesSent totalBytesSent:(int64_t)totalBytesSent totalBytesExpectedToSend:(int64_t)totalBytesExpectedToSend {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (self->latestFaceMapResultCallback != nil) {
            [self->latestFaceMapResultCallback onFaceMapUploadProgress:uploadProgress];
        }
    });
}
@end
